<?php
	...
	$returnvar="";
		...
		foreach($result as $row){
			...
			if($rowcount==1){
				...
				$returnvar.= '<img class="phuto" src="images/'.$row['Image1'].'" style="position:relative;top:16px;height:200px;width:200px;border-radius:50px 50px 150px 150px;">  <img src="images/'.$row['Image2'].'" style="height:310px;width:250px;display:none;"><br><br><br><strong><h3>'.$name.'</h3></strong><br><h4>-'.$publisher.'</h4><br><p>'.$srtdesc.'</p>
				...
				<button type="button" value="'.$row['book_rent_price'].'" id="alertconfirm'.$row['book_id'].'" class="add_to_rent" style="display:none;position:absolute;
    height: 35px;
    background: #1c8adb;
    color: #fff;
    top:400px;
    ...
    border-radius: 4px;
    width:200px;
    left:39px;" onclick="rental('.$row['book_id'].',\''.$name.'\');">Rs. '.$row['book_rent_price'].'</button></div><br><br><br>';
			$idea++;
